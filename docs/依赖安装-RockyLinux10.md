...
本文给出在 Rocky Linux 10 上为 ninfer-4090（含三元移植补丁）补齐缺失系统库的命令，并说明各项依赖的判定依据。

判定依据是 ninfer-4090 自身的 `CMakeLists.txt` 与同仓 `Dockerfile`，并非经验推测；环境实测见第 5 节。
...
其中 `ninja-build` 与 `pkgconf-pkg-config` 在本机已存在，列出这两项是为了命令在全新机器上同样可用（dnf 对已安装的包是幂等的）。

`xgrammar`、`cpp-httplib`、`nlohmann/json`、`utf8proc` **不在上表中**，原因见第 4 节。

也可以直接使用工作区脚本，其中包含安装后自检：
...
版本下限全部来自源码，并非自行选定：`CMakeLists.txt:127` 要求 `libavformat>=60 libavcodec>=60 libavutil>=58 libswscale>=7`，`CMakeLists.txt:131` 要求 `libcurl>=7.85`。

## 3. 两个非显然的门控条件

### 3.1 Linux 下 FFmpeg 为强制依赖，禁用 apps 也无法规避

`CMakeLists.txt` 的非 WIN32 分支中，FFmpeg 查找位于顶层，不在任何 `option` 之内：
...
因此 `-DNINFER_BUILD_APPS=OFF -DBUILD_TESTING=OFF` 仍然会在 configure 阶段因缺少 FFmpeg 而失败。四个 FFmpeg `-devel` 包必须安装。
...
这样可以免于安装 `libcurl-devel`；FFmpeg 的四个 `-devel` 包仍必须安装。

## 4. 表面缺失、实际并不缺失的头文件

对 `src/ apps/ include/ tests/` 下全部 110 个尖括号 include 逐个执行 `g++ -fsyntax-only` 探测，共 39 个在本机无法找到。逐项核对后，仅 9 个确实缺失（第 2 节表中列出者），其余属于探测器自身的假阳性或与平台无关的头文件：
...
| CUDA 13 布局 | `cub/block/block_merge_sort.cuh`、`cuda_pipeline.h` | CUDA 13 将 CUB 移到 `$CUDA/include/cccl/cub`，由 `CUDAToolkit` 的 include 目录提供；探测命令未包含该路径 |
| 构建期拉取 | `xgrammar/compiler.h`、`matcher.h`、`tokenizer_info.h` | 见 4.1 |

### 4.1 xgrammar 不是系统包，无法安装也无需安装

xgrammar 由 `FetchContent_Declare` 在 configure 阶段从 GitHub 获取（`CMakeLists.txt:167-193`，`GIT_TAG v0.2.5.post1`）。它不是 dnf 可解析的依赖，本机 `dnf provides '*/xgrammar/compiler.h'` 返回无匹配。

要求：`git` 已安装（本机 2.52.0），且 configure 阶段可访问 `github.com`。若构建机离线，需预先准备 `FETCHCONTENT_SOURCE_DIR_XGRAMMAR_SOURCE` 指向本地副本。
...
- 若改用 `ffmpeg-free-devel` 元包：**241 个包，下载 114 MB**，因为它连带引入 `libavdevice`/`libavfilter`，进而引入 pipewire、samba、tesseract 等桌面栈

因此命令中有意选用四个 `-free-devel` 子包，而非 `ffmpeg-free-devel`。

## 6. 备选方案

### 6.1 需要完整编解码器时改用 RPM Fusion

EPEL 的 `ffmpeg-free` 是剔除专利受限组件的构建。若媒体采集阶段需要使用 `ffmpeg-free` 未收录的编解码器，改用 RPM Fusion 的完整 `ffmpeg-devel`：
...
构建阶段除系统包外还依赖网络（xgrammar 获取）与 WebUI 资源下载（`NINFER_ENABLE_UI` 默认 ON，可从 GitHub release 获取；离线时 CMake 会发出告警并退化为 UI stub）。

### 7.1 用 uv tool install 安装时

`uv tool install` 使用的正是这条构建链路 —— 它在 wheel 构建阶段克隆上游源码并编译，因此上述
5 个系统包缺一不可，此外还需要：

| 额外依赖 | 用途 | 备注 |
|---|---|---|
| `uv` | 安装器 | 可用官方脚本一条命令安装，见下 |
| CUDA Toolkit（含 `nvcc`） | 编译三元内核 | 需与本机驱动匹配 |
| `git` | 获取上游与 xgrammar | 见 [把本仓当工具用](uv-工具安装.md) |

    curl -LsSf https://astral.sh/uv/install.sh | sh

网络不可达时，可以先在一台联网机器上执行 `uv build --wheel`，再将 wheel 复制到目标机安装；
...
- 完整 CMake 构建（含 xgrammar 获取与 UI 下载）尚未在本机成功执行，依赖安装完成前无法进行。
