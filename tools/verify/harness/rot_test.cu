...
//
// Build（Linux）：
//   nvcc -O2 -std=c++20 -arch=sm_89 -I <ninfer>/src -o rot_test rot_test.cu
...
void write_f32(const std::string& name, const std::vector<float>& data) {
    const std::string path = std::string(out_dir()) + name;
...
void run_case(const Case& c, std::uint32_t seed) {
    ...
    std::printf("case %-10s k=%-6d tokens=%d n_blk=%d perm=(%d,%d,%d) inverse=%d  -> %s%s\n",
                ...
                c.inverse ? 1 : 0, out_dir(), tag.c_str());
...
int main() {
    // prop 必须活到 printf 之后：旧写法在 lambda 里返回 prop.name，是"返回局部变量地址"的未定义行为。
    cudaDeviceProp prop{};
    if (cudaGetDeviceProperties(&prop, 0) != cudaSuccess) { std::snprintf(prop.name, sizeof(prop.name), "unknown"); }
    std::printf("device: %s\n", prop.name);
